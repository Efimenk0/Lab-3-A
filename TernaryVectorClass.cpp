...
namespace TernaryVectorClass {
    ...
    TernaryVector::TernaryVector(int X) {
        if (X > SZ)
            throw std::invalid_argument("Неверное кол-во неопределенных значенийю Попробуйте еще раз.\n");
        for (int i = 0; i < X; ++i)
            vector[i] = 'X';
        top = X;
    }
    ...
    TernaryVector::TernaryVector(const std::string& str) {
        ...
        if (size > SZ)
            throw std::invalid_argument("Размер строки превышает максимальный размер вектора. Попробуйте еще раз.\n");
        str_i = 0;
        top = size;
        int i = 0;
        while (i < size) {
